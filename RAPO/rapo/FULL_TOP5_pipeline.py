...
def bitcode_is_valid(bc: Path) -> bool:
    if not bc or not Path(bc).exists():
        return False
    try:
        r = subprocess.run(
            ["llvm-dis", str(bc), "-o", os.devnull],
            capture_output=True,
            text=True,
            timeout=30,
        )
...
def apply_seq_count(input_bc: Path, seq_txt: str) -> Tuple[Optional[int], float]:
    ...
    try:
        ...
        try:
            ...
            ok = False
            print(f"[apply_seq_count] error applying seq: {e}")
...
def load_cluster_top5_sequences(cluster_analysis_csv: Union[str, Path]) -> Dict[int, List[str]]:
    df = pd.read_csv(cluster_analysis_csv)
    cluster_col = None
    for cand in ["cluster_id", "cluster", "centroid"]:
    ...
    if cluster_col is None:
        raise ValueError("cluster_analysis csv must have 'cluster_id'/'cluster'/'centroid'")
    if "top_5_closest_ids" not in df.columns:
        raise ValueError("cluster_analysis csv must have 'top_5_closest_ids'")
    if "closest_code_id" not in df.columns:
        raise ValueError("cluster_analysis csv must have 'closest_code_id'")

    m: Dict[int, List[str]] = {}
    for _, r in df.iterrows():
        cid = int(r[cluster_col])
        raw = str(r["top_5_closest_ids"]) if pd.notna(r["top_5_closest_ids"]) else ""
        toks = [t.strip() for t in raw.split(",") if t.strip()]
        uniq: List[str] = []
        for token in toks:
            normalized_token = normalize_id(token)
            if normalized_token not in uniq:
                uniq.append(normalized_token)
            if len(uniq) >= 5:
                break
        if not uniq:
            uniq = [normalize_id(r["closest_code_id"])]
        m[cid] = uniq
    return m


def _apply_seq_task(input_bc: str, seq_code_id: str, seq_base_dir: Union[str, Path]) -> Dict:
    seq_txt = read_sequence_text_for_code_id(seq_code_id, base_dir=seq_base_dir)
    if not seq_txt or not input_bc:
        return {"seq_id": seq_code_id, "count": None, "compile_ms": 0.0}
    try:
        cnt, comp_ms = apply_seq_count(Path(input_bc), seq_txt)
    except Exception as e:
        print(f"[apply_seq_task] seq_id={seq_code_id} error: {e}")
        cnt, comp_ms = None, 0.0
    return {"seq_id": seq_code_id, "count": cnt, "compile_ms": comp_ms}


def _centroid_worker_process(
    task: Dict,
    cluster_top5: Dict[int, List[str]],
    seq_base_dir: Union[str, Path],
    inner_threads: int,
) -> Dict:
    sid = task["id"]
    cid = task["cluster"]
    seq_ids = cluster_top5.get(cid, [])
    oz = count_oz_for_sample(sid)
    inp = ensure_bitcode_for_sample(sid)
    if inp is None or not seq_ids or not bitcode_is_valid(inp):
        return {
            "id": sid,
            "cluster": cid,
            "best_code_id": "",
    ...
    futs = []
    with ThreadPoolExecutor(max_workers=min(inner_threads, len(seq_ids))) as tex:
        for code_id in seq_ids:
            futs.append(tex.submit(_apply_seq_task, str(inp), code_id, seq_base_dir))
    ...
    for r in results:
        if r["count"] is not None and r["count"] > 0 and (best_cnt is None or r["count"] < best_cnt):
            best_cnt = r["count"]
            best_code = str(r["seq_id"])
        try:
            compile_ms_total += float(r["compile_ms"])
        except Exception:
            pass

    ratio = None
    reduction_pct = None
    if oz and best_cnt is not None and oz > 0:
        ratio = best_cnt / float(oz)
        reduction_pct = (1.0 - ratio) * 100.0

    return {
        "id": sid,
        "cluster": cid,
        "best_code_id": best_code,
        "oz_count": oz if oz is not None else "",
        "centroid_count": best_cnt if best_cnt is not None else "",
...
def run_centroid_top5_application(
    ...
    log_every = 10
    print(f"[centroid-top5] outer workers={workers}, inner threads={inner_threads}, samples={total_samples}")

    tasks = [{"id": str(r.id), "cluster": int(r.cluster)} for r in assignments_df.itertuples()]

    with ProcessPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(_centroid_worker_process, t, cluster_top5, seq_base_dir, inner_threads): t["id"] for t in tasks}
        done = 0
        for fut in as_completed(futs):
            rows.append(fut.result())
            done += 1
            if done % log_every == 0 or done == total_samples:
                print(f"[centroid-top5] {done}/{total_samples} samples completed")

    elapsed_ms = (time.perf_counter() - t0) * 1000.0
    rows.sort(key=lambda d: int(d["id"]))
    df = pd.DataFrame(rows, columns=["id", "cluster", "best_code_id", "oz_count", "centroid_count", "reduction_pct", "compile_ms"])
...
def _apply_seq_task_with_cluster(input_bc: str, seq_code_id: str, target_cluster: int, seq_base_dir: Union[str, Path]) -> Dict:
    seq_txt = read_sequence_text_for_code_id(seq_code_id, base_dir=seq_base_dir)
    if not seq_txt or not input_bc:
        return {"seq_id": seq_code_id, "cluster": target_cluster, "count": None, "compile_ms": 0.0}
    try:
        cnt, comp_ms = apply_seq_count(Path(input_bc), seq_txt)
    except Exception as e:
        print(f"[apply_seq_task_with_cluster] seq_id={seq_code_id} cl={target_cluster} error: {e}")
        cnt, comp_ms = None, 0.0
    return {"seq_id": seq_code_id, "cluster": target_cluster, "count": cnt, "compile_ms": comp_ms}


def _fallback_worker_sample(
    row: Dict,
    cluster_top5: Dict[int, List[str]],
    ...
    base_code = row.get("best_code_id", "")

    if not (pd.notna(oz) and oz > 0):
        oz = count_oz_for_sample(sid)

    inp = ensure_bitcode_for_sample(sid)
    if inp is None or not bitcode_is_valid(inp):
        return {
            "id": sid,
            "best_cluster": assigned_cluster,
            "best_code_id": base_code,
            "best_count": base_cnt if base_cnt is not None else np.nan,
            "best_reduction_pct": np.nan,
            "best_below_p10": False,
            "extras_compile_ms_total": 0.0,
        }

    t5 = str(row.get("top_5_closest_clusters", "") or "")
    clusters = [c.strip() for c in t5.split(",") if c.strip().isdigit()]
    extras = clusters[1:5] if len(clusters) >= 5 else clusters[1:]

    seq_jobs: List[Tuple[int, str]] = []
    for c in extras:
        try:
            cid = int(c)
        except Exception:
    ...
    futures = []
    extras_compile_ms = 0.0
    best_cnt = base_cnt
    best_code = base_code
    best_cluster = assigned_cluster
    with ThreadPoolExecutor(max_workers=min(inner_threads, len(seq_jobs))) as tex:
        for cid, code_id in seq_jobs:
            futures.append(tex.submit(_apply_seq_task_with_cluster, str(inp), code_id, cid, seq_base_dir))
        for fut in futures:
            try:
                res = fut.result()
            except Exception:
                continue
            try:
                extras_compile_ms += float(res.get("compile_ms", 0.0))
            except Exception:
                pass
            cnt = res.get("count")
            if cnt is not None and cnt > 0 and (best_cnt is None or cnt < best_cnt):
                best_cnt = float(cnt)
                best_cluster = res.get("cluster", best_cluster)
                best_code = str(res.get("seq_id", ""))
    ...
    return {
        ...
        "extras_compile_ms_total": extras_compile_ms,
    }


def run_fallback_stage_top5(
    ...
    fallback_rows = []
    t0 = time.perf_counter()
    total_samples = len(ncs)
    log_every = 5
    print(f"[fallback-top5] outer workers={workers}, inner threads={inner_threads}, samples={total_samples}")

    tasks = [r._asdict() if hasattr(r, "_asdict") else r.__dict__ for r in ncs.itertuples()]
    with ProcessPoolExecutor(max_workers=workers) as ex:
        futs = {ex.submit(_fallback_worker_sample, t, cluster_top5, p10_map, seq_base_dir, inner_threads): t["id"] for t in tasks}
        done_samples = 0
        for fut in as_completed(futs):
            fallback_rows.append(fut.result())
            done_samples += 1
            if done_samples % log_every == 0 or done_samples == total_samples:
                print(f"[fallback-top5] {done_samples}/{total_samples} samples completed")
    fallback_elapsed_ms = (time.perf_counter() - t0) * 1000.0

    fallback_df = pd.DataFrame(fallback_rows)
    ncs = ncs.merge(fallback_df, on="id", how="left")
    # Ensure expected columns exist even if fallback_df is empty
    for col, default in [
        ("best_code_id", ""),
        ("best_cluster", np.nan),
        ("best_count", np.nan),
    ...
    final = final.merge(
        ncs[["id", "best_cluster", "best_code_id", "best_count", "best_reduction_pct", "best_below_p10", "below_p10"]],
        on="id",
        how="left",
    )
...
def main():
    ap = argparse.ArgumentParser(description="Top-5 centroid/fallback pipeline")
    ...
    ap.add_argument("--oz_centroid_out", type=str, default="oz_centroid_5k_ppo_top5.csv")
    ap.add_argument("--fallback_out", type=str, default="oz_centroid_5k_ppo_fallback_top5.csv")
    ...
    ap.add_argument("--centroid_inner_threads", type=int, default=5)
    ap.add_argument("--fallback_inner_threads", type=int, default=20)
    ...
    if args.centroid_inner_threads <= 0:
        ap.error("--centroid_inner_threads must be positive.")
    ...
    print(
        f"[info] centroid outer workers={args.workers}, "
        f"inner threads={args.centroid_inner_threads}; "
        f"fallback outer workers={fallback_workers}, "
        f"inner threads={args.fallback_inner_threads}."
    )
    ...
    cluster_top5 = load_cluster_top5_sequences(args.cluster_analysis_csv)

    # Step 2: apply top-5 centroid sequences
    centroid_df, gm_5k_ratio, centroid_ms = run_centroid_top5_application(
        assignments_df,
        cluster_top5,
        args.seq_base_dir,
        args.oz_centroid_out,
        workers=args.workers,
        inner_threads=args.centroid_inner_threads,
    )
    ...
    if gm_5k_ratio is None:
        ...
        print(f"(1) Geomean reduction_pct (5k, centroid best-of-5) : {(1.0 - gm_5k_ratio) * 100.0:.4f}%")

    # Step 3: fallback with top-5 sequences per extra cluster
    fallback_final_df, fb_stats = run_fallback_stage_top5(
        centroid_df,
        top5_csv=args.top5_csv,
        cluster_p10_csv=args.cluster_p10_csv,
        cluster_top5=cluster_top5,
    ...
    print(f"(3) Geomean for NCS after top2-5 clusters*5 seqs: {fb_stats.get('gm_ncs_best'):.4f}%") if fb_stats.get("gm_ncs_best") is not None else print("(3) Geomean for NCS after top2-5 clusters*5 seqs unavailable")
